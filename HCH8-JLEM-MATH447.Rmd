...
#data
hardata = data.frame(A,B,C,D,E,F,G,H,J,low.c)
...
```{r, fig.show="hide"}
#half normal probability plot
qqnorm(aov(low.c ~ A*B*C*D*E*F*G*H*J, fraction.hardata), label = TRUE)#F and D
```
...
```{r}
#Refined Model
hardata.lm2.c <- lm(low.c ~ D*F, hardata)
...
#half normal probability plot
qqnorm(aov(sqrtlow.c ~ A*B*C*D*E*F*G*H*J, fraction.hardata), label = TRUE)#F and D

#Refined Model
hardata.lm.sqrtc2 <- lm(sqrtlow.c ~ D*F, hardata)
