...
#fraction.hardata=with(hardata, hardata[A * B * C * D * E * G == 1,])
...
qqnorm(aov(FTMOD ~ A*B*C*D*E*F*G*H*J, hardata), label = TRUE)#F and AG
```
...
```{r}
#refined model
hardata.lm2 <- lm(FTMOD ~ F * A*G, fraction.hardata); summary(hardata.lm2)
```
